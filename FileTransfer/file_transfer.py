...
#Use GUI to act as the interface for users to transfer files from one directory to another. 
    ...
    def transferFiles(self):
        ...
        for i in source_files:
            filepath = os.path.join(source, i)
             #Gets new or last modified files within the last 24 hours.
            timestamp = os.path.getmtime(filepath)
            #convert timestamp into DateTime object
            datestamp = datetime.fromtimestamp(timestamp)
            #moves each file from the source to the destination
            if (datestamp > onedayago):
